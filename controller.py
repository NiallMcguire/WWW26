...
def main():
    ...
    config = {
        ...
        'test_samples': len(test_dataloader.dataset),  # ADD THIS LINE
    ...
    print(f"Test samples: {config['test_samples']}")  # ADD THIS LINE
    ...
    model = create_model(
        ...
        ablation_mode=args.ablation_mode,  # ADD THIS
        ablation_match_params=args.ablation_match_params  # ADD THIS
    )
    ...
    # ADD TEST EVALUATION (after model saving, before finish_wandb())
    ...
    if args.enable_test_evaluation:
        ...
        test_results = test_model(
            model=trained_model,
            test_dataloader=test_dataloader,  # NOW USING ACTUAL TEST SET
            device=device,
            debug=args.debug,
