...
class LaBramChannelPatcher(nn.Module):
    """TRUE LaBraM channel-wise patching (Jiang et al., ICLR 2024)"""
    ...
    def forward(self, eeg_input):
        ...
        all_patches, channel_indices, time_indices = [], [], []

        for word_idx in range(num_words):
            word_data = eeg_input[:, word_idx, :, :]
            word_patches = []
            for ch_idx in range(channels):
                channel_data = word_data[:, :, ch_idx]
                for t_idx in range(num_time_patches):
                    start_t, end_t = t_idx * self.patch_length, (t_idx + 1) * self.patch_length
                    if end_t <= time_samples:
                        patch = channel_data[:, start_t:end_t]
                        word_patches.append(patch.unsqueeze(1))
                        if word_idx == 0:
                            channel_indices.append(ch_idx)
                            time_indices.append(t_idx)
            if word_patches:
                all_patches.append(torch.cat(word_patches, dim=1))
...
class LaBramCNNPreprocessor_TRUE(nn.Module):
    """TRUE LaBraM single-channel CNN (Jiang et al., ICLR 2024)"""

    def __init__(self, hidden_dim=768, num_layers=3):
        super().__init__()
        if num_layers == 3:
            self.cnn_blocks = nn.Sequential(
                nn.Conv1d(1, hidden_dim // 4, kernel_size=15, stride=8, padding=7),
                nn.GroupNorm(4, hidden_dim // 4), nn.GELU(),
                nn.Conv1d(hidden_dim // 4, hidden_dim // 2, kernel_size=3, stride=1, padding=1),
                nn.GroupNorm(8, hidden_dim // 2), nn.GELU(),
                nn.Conv1d(hidden_dim // 2, hidden_dim, kernel_size=3, stride=1, padding=1),
                nn.GroupNorm(16, hidden_dim), nn.GELU()
            )
        else:
            self.cnn_blocks = nn.Sequential(
                nn.Conv1d(1, hidden_dim, kernel_size=15, stride=8, padding=7),
...
class LaBramPositionalEmbeddings(nn.Module):
    """TRUE LaBraM positional embeddings (Jiang et al., ICLR 2024)"""
    ...
    def __init__(self, hidden_dim, max_words=50, max_time_patches=50, max_channels=128):
        ...
        nn.init.trunc_normal_(self.temporal_embeddings.weight, std=0.02)
        nn.init.trunc_normal_(self.spatial_embeddings.weight, std=0.02)
        nn.init.trunc_normal_(self.word_embeddings.weight, std=0.02)

    def forward(self, channel_indices, time_indices, word_indices):
        batch_size = word_indices.shape[0]
        temporal_emb = self.temporal_embeddings(time_indices).unsqueeze(0).expand(batch_size, -1, -1)
        spatial_emb = self.spatial_embeddings(channel_indices).unsqueeze(0).expand(batch_size, -1, -1)
        word_emb = self.word_embeddings(word_indices)
        return temporal_emb + spatial_emb + word_emb
...
class SimplifiedBrainRetrieval(nn.Module):
    ...
    def _apply_labram_preprocessing(self, eeg_input):
        ...
        if self.use_sequence_concat:
            return patch_features.reshape(batch_size, num_words * num_patches, self.hidden_dim), 'sequence'
        else:
            return patch_features.mean(dim=2), 'word'
